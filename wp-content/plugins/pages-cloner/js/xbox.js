window.XBOX = (function(window, document, $){
  ...
  xbox.init = function() {
    ...
    $xbox.on('input', '.xbox-type-textarea textarea.xbox-element', xbox.on_change_textarea );
  ...
  xbox.sticky_submit_buttons = function(){
    $( window ).scroll(function() {
      var $header = $('.xbox-header');
      var $actions = $header.find('.xbox-header-actions');
  ...
  xbox.load_icons_for_icon_selector = function( event ){
    ...
    $('.xbox').on('input', '.xbox-search-icon', function(event) {
      event.preventDefault();
      var value = $(this).val();
      var $container = $(this).closest('.xbox-field').find('.xbox-icons-wrap');
      xbox.filter_items(value, $container, '.xbox-item-icon-selector');
    });
    $('.xbox').on('click', '.xbox-icon-actions .xbox-btn', function(event) {
      var value = $(this).data('search');
      var $container = $(this).closest('.xbox-field').find('.xbox-icons-wrap');
      xbox.filter_items(value, $container, '.xbox-item-icon-selector');
    });

    $('.xbox').on('click', '.xbox-icons-wrap .xbox-item-icon-selector', function(event) {
      ...
      success: function( response ) {
        if( response ){
          if( response.success ){
            ...
            var icon_font_style = {};
            icon_font_style['font-size'] = parseInt(options.width)/2;
            icon_font_style['line-height'] = (parseInt(options.height) - 14) + 'px';
            $field.find('.xbox-icons-wrap .xbox-item-icon-selector i').css(icon_font_style);
  ...
  xbox._remove_group_item = function( $btn ){
    ...
    if ( $group_wrap.children('.xbox-group-item').length > 1 ) {
      ...
      $item.fadeOut(200, function(){
        ...
        $row.trigger('xbox_after_remove_group_item', [index]);
      });
    }
  };
  ...
  xbox.set_field_value = function( $field, value, extra_value ){
    var $input, array;
    ...
    switch(type){
      case 'number':
        ...
        $field.find('input.xbox-unit-number').attr('value', extra_value).trigger('change');
        unit = unit === '' ? '#'  : unit;
      ...
      case 'select':
        ...
        if( max_selections > 1 && $dropdown.hasClass('multiple') ){
          $dropdown.dropdown( 'set selected', value.split(',') );
        } else {
